...
本檔是「一律適用」規則的唯一權威，共 21 節。**刻意不拆檔**——拆成兩份會讓讀者為了一條規則翻兩個地方，違反「規則只住一個地方」。用下表跳轉：
...
| [交通工具的取得方式要給結論](#car-plan) | 租車／共享車怎麼比價、怎麼給推薦 |
...
const trip = {
  ...
  // 租車／共享車的比價與推薦（見 [#car-plan](#car-plan)）。自駕行程才需要；
  // 大眾運輸為主的行程用 transit 取代（conditional-features.md#transit）。
  carPlan: { title, lede, recommendation, compare, carshare, documents, roadRules, transfer },
  ...
  days: [
    {
      ...
      // 完整規格與 class 見 conditional-features.md#alternatives。
...
- **住宿的訂房連結也走這個機制，不要用 `actionLink`。** `hotelAreas[].options[]` 接受同一組 `bookingUrl` / `bookingKind` / `leadDays`，徽章渲染在旅館名稱後面。兩者的界線是**來源**：自己查證來的官方訂房頁是 `bookingUrl`；`actionLink` 只留給使用者已裝的官方 skill 回傳的 deeplink（`page-contract.md#action-links`）。
  > 這一條是踩過才補的：`bookingUrl` 機制先做在時間軸站點上，卻忘了住宿——而住宿是整趟最需要訂的東西。資料裡放了 `actionLink`，頁面完全沒讀，也沒有任何檢查發現。**新增一種連結欄位時，要問「還有哪裡也會用到它」。**
- **要跟 `trip.reminders` 對得上。** 同一件事出現在兩個地方（頁頂清單、時間軸徽章），兩邊必須說一樣的話。只在其中一邊出現，讀者就會以為另一邊漏了。

## 交通工具的取得方式要給結論，不是給清單 {#car-plan}

行程頁常見的失敗是「把租車業者列一排，價格寫個區間，讓讀者自己選」。那不是規劃，是把工作推回去。**`trip.carPlan` 的規格是：先給一個明確推薦，再給比價表證明那個推薦是怎麼來的。**

```js
trip.carPlan = {
  title: '租車與共享車：怎麼租、租幾天、比價與推薦',
  lede:  '價格是 2026/08/23 查到的公開牌價，未經訂位引擎報價，僅供抓預算。',   // 價格的時效與性質要講明
  recommendation: {
    choice:  '市區 Avis（120 10th Ave SW，店號 S50）取還，只租 10/4–10/10 共 7 天',  // 具體到業者與地點
    why:     '…',            // 為什麼是這個
    caution: '…',            // 這個方案自帶的風險（如「還車不能拖到假日」）
    totalCAD:'約 CA$1,100–1,400（7 天，三人合計，含稅費油錢）',
    breakdown: [ { item, cad, note } ],      // 成本拆解，含「不用付」的項目
    howTo:   '…',            // 實際怎麼訂、怎麼自己驗證價差
  },
  compare:   [ { option, priceCAD, pros, cons, verdict } ],
  carshare:  { verdict, detail, useful, howTo, caution },
  documents: [ { item, detail, gotcha } ],
  roadRules: [ { item, detail, gotcha } ],
  transfer:  [ { mode, priceCAD, time, note } ],
};
```

### 四條紀律

- **推薦要具體到「跟誰租、哪個地點、租幾天」。** 「建議在市區租車」不是推薦，「市區 Avis S50、10/4–10/10 七天」才是。少了地點與天數，讀者還是得自己做決定。
- **成本拆解要含「不用付」的項目。** 寫上「亞伯達 6% 租車稅：CA$0，2027/1/1 才上路」比不寫更有價值——它擋掉讀者在別處看到那 6% 之後的焦慮，也說明了預算為什麼比網路上的攻略低。
- **租車天數要跟逐日行程對齊。** 取車與還車本身就是時間軸上的站點（含座標與連結）。「只租 7 天」這個結論如果沒有反映在 10/4 的取車站與 10/10 的還車站上，讀者照著行程走就會發現對不上。
- **共享車要給「不能用」的理由，不能只說不推薦。** Communauto 卡加利只有單程 FLEX、行程必須在市中心 Zone 內結束、每趟只含 75 km——這三條是結構性的，寫出來讀者才知道不是價格問題，也才知道它在市區那幾天仍然有用。

### 這一節容易寫錯的地方

| 陷阱 | 說明 |
|---|---|
| 拿比價站的地址當真 | 比價站會長期掛著已經關閉的據點。Budget 卡加利市區店早就沒了，Expedia 與 Travelocity 到現在還列著兩個地址——照那個地址去取車，出發日早上會發現店不存在 |
| 把「每趟含 75 km」讀成每天 | 共享車的里程額度多半是 per trip。多日行程整段只送 75 km，其餘全部按公里計費，成本會爆掉 |
| 只比日租價 | 加拿大機場租車的稅費堆疊很兇，只看日租價會低估實付 30–40%。比價一定要比訂車最後一頁的 Estimated Total |
| 漏掉附加駕駛費 | 多人輪流開是常態，但那是每人每天 CA$10–13 的真金白銀，長租下來比日租價差還大 |
| 忘記假日 | 市區據點週末與國定假日常常不營業。還車日撞到假日，整個「市區取還省錢」的方案就破了 |
