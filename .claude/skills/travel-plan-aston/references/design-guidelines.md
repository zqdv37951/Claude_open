...
## 預訂徽章的視覺 {#booking-visuals}

`reminders.js` 產出 `.reminder-badge` 加上 `.ticket` / `.reserve` / `.permit`，結構由 `assets/page-contract.md#booking` 定死，這裡只管長什麼樣：

- **它要看起來可以點。** 有 `url` 時渲染成 `<a>`，必須跟同一行的 `.pill`（時段標籤）明顯不同——底線、主色、或明顯的邊框都可以，但不能只是換個底色，那會被讀成又一個靜態標籤。
- **三種 kind 的權重：`ticket` ≈ `reserve` > `permit`。** 前兩者是「不做這件事可能就去不成」，通行證通常買一張走全程，屬於背景資訊。
- **不要用跟 `.status.season`（季末警示）同一個顏色。** 兩者都會出現在同一頁，甚至同一張卡上：「10/12 收季」是**時間**壓力，「需購票」是**行動**要求，讀者要能一眼分開。季末已經佔了警示色，預訂徽章走主色系。
- **跟時間軸的 `.tl-num` 編號徽章拉開距離。** 同一行裡有編號、標題、地圖連結、預訂徽章、時段標籤五個東西，最容易變成一團。給 `h4` 足夠的 `gap`，並讓預訂徽章是那一行裡唯一帶邊框或底線的元素。
