...
3. 组织成 `assets/page-contract.md` 里定义的 `trip` 数据结构（含可选的 `dataSources` 与节点 `actionLink`）。**若是不涉及航班/住宿的单日往返或国内短程行程**，改读 `references/day-trip-enhancements.md`：用 `trip.transit` 取代 `flights`/`hotelAreas`，并视需要加入 `trip.highlights`（特色景点/美食/店家总览，各至少10项）、每站「附近推荐」toggle 面板、`trip.glossary`（外文名词注解）。
