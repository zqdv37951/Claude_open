...
> 实测下来读者期待的阅读动线：行前须知 → 交通/航班+酒店 → **时间序行程（含地图）** → 延伸参考资料（如 `highlights` 总览）→ 免责声明 → 全程贴士。时间序行程本身**不应排在**延伸参考资料之后，它才是主要内容。**不限行程长短或国内/跨国**，一律参照 `references/trip-extras.md`（`highlights` 总览、每站附近推荐、站点标题连结、外文名词注解等在该文件里是无条件规则，唯独「不涉及航班/住宿时用 `transit` 取代本节」才是条件式的）。
