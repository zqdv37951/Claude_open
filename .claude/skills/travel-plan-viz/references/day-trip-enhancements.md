...
時間序行程表（含地圖）**必須排在** highlights 總覽之前——total 總覽是延伸參考資料，不是主要內容，不該搶在行程本身前面。
...
`design-guidelines.md` 的內建美學準則是「沒有更好參考時的兜底方案」；若使用者提供了另一份（同一 skill 產生或其他來源的）頁面並表示「喜歡這個配色」，**直接沿用對方的 CSS 變數命名與數值**（含淺色／深色雙主題的三段式寫法：裸 `:root` 定義淺色、`@media (prefers-color-scheme: dark)` 搭配 `:not([data-theme="light"])`、`:root[data-theme="dark"]` 三層），不要自創一套再混用兩邊的變數名稱——這是這幾輪修改中唯一一次「使用者的話」明確蓋過內建準則優先權的案例，須完整套用，不要只挑喜歡的幾個顏色片段。
...
- 第 7 節建議併入 `design-guidelines.md`，補一條「使用者提供參考頁面時的優先權」規則。
