...
| [`roumanm-adblock-lazyload.user.js`](roumanm-adblock-lazyload.user.js) | Network-Level Ad Blocker + Conditional Lazy Load Trigger (Universal Pro) | 全站攔截指定廣告網域(fetch/XHR/`src`/DOM 節點),並在 `/books/` 閱讀頁自動滾到底部再跳回原位,觸發懶加載 | `*.roumanm.com` |
...
- `csdn-greener.user.js`、`zhihu-enhancement.user.js` 為安裝自 GreasyFork
  ...
  此備份檔。
- 其餘四支(Amazon、Salesforce、KCC、roumanm)為個人針對特定網站流程撰寫的
  自動化腳本,內容可能含公司/服務名稱等識別資訊,依需求自行調整或移除。
